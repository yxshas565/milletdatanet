...
def train_one_fold(train_rows, val_rows, device):
    ...
    train_labels = [CLASS_TO_IDX[r['growth_stage']] for r in train_rows]
    ...
    train_loader = DataLoader(LeafDataset(train_rows, train_tf), batch_size=8, sampler=sampler)
...
def main():
    ...
    print("\n=== AUGMENTED RESULTS (class-weighted + oversampled, 5-fold CV) ===")
    ...
    torch.save(last_model.state_dict(), "ml/models/augmented_resnet18.pt")
    os.makedirs("docs", exist_ok=True)
    with open("docs/augmented_results.json", "w") as f:
        json.dump({"model": "resnet18_augmented_5fold", "classes": CLASSES,
